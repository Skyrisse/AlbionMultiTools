...
Локальная утилита-оверлей для **Albion Online**. Распознаёт текущую зону со скриншота (офлайн OCR), показывает информацию о локации в оверлее (можно без оверлея, отображение будет прям в приложении), считает фейм и делёжку серебра. Работает без интернета — никакая телеметрия и данные пользователя никуда не отправляются.
...
- **Оверлей** — Albion-стилизованная панель с иконкой зоны, списком объектов и легендой; автозакрытие, прозрачность, масштаб, смещение по X/Y (опционально можно отключить)
...
- **Делёжка** — список отсканированных значений серебра (можно сканировать с сундуков как зоны по той же горячей клавише или из инвентаря), сумма и деление на N человек

### БИЛДЫ
- Встроенный редактор билдов
